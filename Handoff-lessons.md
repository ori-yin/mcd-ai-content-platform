...
### 22. 性能优化"按 ROI 排序 + 每次改完立即测"——P0/P1 拆档、不破回归才算完成（2026-09-03 · Phase 49）

**坑**：Phase 48 性能诊断后列了 8 个潜在优化点（A/B/C/D/E/F/G/H/I），**如果一次全改**：
- 改完如果出问题，不知道是哪个优化导致的（debug 时间 × N）
- 优化顺序错乱可能导致**前面的优化掩盖后面的效果**（如先做 A hx-boost 让服务端 bench 数据变正常，再做 B 关 auto_reload 的效果被淹没）
- 单次 commit 包含太多变更，git revert 时粒度太粗

**根因**：性能优化是**实验性改动**——每次干预的效果需要**独立验证**，否则就是"凭印象觉得快了"。

**铁律**：
1. **优化前先列清单 + 按 ROI 排序**（按用户感知收益 ÷ 实施成本 ÷ 风险）—— Phase 49 把 8 项排成 P0/P1/P2/P3 四档
2. **每改一项立即测**：bench + 848 回归 + 视觉/curl 验证（如 D 必跑 `curl -I` 确认头，A 必 grep `<body hx-boost` 确认）
3. **改坏的早回滚**：单次改 1 个变量，出问题定位 + revert 都是 1 分钟
4. **失败的改动要诚实标注**：B 改 cold 12.4s → 1.2s（-90%）是真实收益；C 改 cold 仍 1.2s（不解决 ASGI 启动开销）—— 都要在 commit message + Handoff 里如实写，不夸大不缩水
5. **不改就明说**：A hx-boost bench 测不出，但浏览器侧感官提升最大——这种"bench 无收益但用户感知有收益"的改动要单独说清楚

**避坑指引**：
- 优化前先列清单（按"预估收益 / 实施成本 / 是否需实测"3 列）—— 用户可挑，不是技术员单方面决定
- **基线先测 1 次**（不带任何改动），记录 cold/warm median，后续每次改后**对照基线**而不是对照"上一次"
- **改完 1 个就 commit 1 次**（即使同 phase）—— git log 清晰，回滚粒度细；不要攒到 phase 末一次 commit
- **848 PASS 不破 ≠ 优化有效** —— 必须有 bench / curl / grep 三类证据链
- **优化延后项要写进 TODO**：F SQLite 索引 / G L1 重复预热 / H lazy import / I 死链清理 —— 不要"想起来再做"

**对照实验数据**（Phase 49 实测）：
- 基线 → B：cold /studio 12444ms → 1226ms（-90%，B 唯一有效改 cold）
- B → C：cold /studio 1226ms → 1215ms（C 无效，1.2s 不是字典；trace_01b 证明 `_01_context` 内部只 2ms）
- C → D：warm / 15.6ms → 4.0ms（D 有效改 warm）；cold /studio 1215ms → 4498ms（D 中间件多 3ms 但不可见）
- D → A：bench 测不出服务端变化（A 是浏览器侧），但 `grep '<body hx-boost="true">'` 确认属性已加
