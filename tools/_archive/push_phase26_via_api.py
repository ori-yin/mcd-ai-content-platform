# -*- coding: utf-8 -*-
"""push_phase26_via_api.py — Phase 26 推送（Git Data API · 2 commits 依次推）
...
本脚本特殊处理：
- 2 个本地 commit 依次推（保留 history）
- 每次推完，parent = 上次创建的 commit（不是远端 HEAD）
- 删除文件走 tree entry sha: null（Contents API 不能删）

用法：
    python tools/push_phase26_via_api.py --token ghp_xxx
"""
...
COMMITS_TO_PUSH = ["30fe4e2", "0a29abc"]
