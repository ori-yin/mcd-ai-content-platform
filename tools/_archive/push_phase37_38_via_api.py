# -*- coding: utf-8 -*-
"""push_phase37_38_via_api.py — Phase 37 + Phase 38 推送（Git Data API · 3 commits 依次推）
...
本脚本特殊处理：
- 3 个本地 commit 依次推（保留 history）：bc198d5 (baseline) → 1d7dd0e (9 BUG) → 77e5249 (Handoff)
- 每次推完，parent = 上次创建的 commit（不是远端 HEAD）
- 删除文件走 tree entry sha: null（Contents API 不能删）

用法：
    python tools/push_phase37_38_via_api.py --token ghp_xxx
"""
...
# 要推的 commits（按从旧到新的顺序；本地 HEAD = 0a29abc → ahead 远端 2 commit）
COMMITS_TO_PUSH = ["bc198d5", "1d7dd0e", "77e5249"]
